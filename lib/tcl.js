...
/**
*   Expose a Javascript callback as a new command in the root Tcl interpreter.
*   This will use the root Tcl Interpreter and can share data between two
*   executions (e.g. global variable, loaded libraries etc.)
*
*   @param {string} cmdName - name of new Tcl command
*   @param {function} cb - Javascript function
*   @return {Result} nothing
*   @throws {Error} Error object containing error data
*/
Tcl.prototype.expose = function ( cmdName, cb ) {
	if (cmdName != "" && typeof(cb) == 'function') {
		this._binding.expose( cmdName, cb );
	} else {
		throw new Error('wrong usage: expose(cmdName, function)');
	}
};
...
Tcl.prototype.$inject = function () {
	...
	var self = this;
	var cmds = this._binding.cmdSync( 'info commands' );
...
*          the response
*/
Tcl.prototype.queue = function ( arg0, arg1, arg2 ) {
	var argv, callback;
	var cmd = (arg0.__proto__.constructor.name == 'Array') ? arg0 : this._binding.toArray(arg0);
	switch (arguments.length ) {
		case 2:
			// command string is the sole argument
			argv = cmd;
			callback = arg1;
			break;
	 	case 3:
			argv = cmd.concat(arg1);
			callback = arg2;
			break;
 	}

	this._binding.queue( argv, function ( err, data ) {
